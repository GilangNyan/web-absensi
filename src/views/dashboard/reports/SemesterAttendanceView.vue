<template>
  <div class="flex flex-col space-y-4">
    <h1 class="font-bold text-indigo-900 text-2xl">{{ t('title.semesterAttendance') }}</h1>
    <BreadCrumbs />
  </div>
  <Form
    class="bg-white p-4 rounded-lg flex flex-col space-y-2"
    id="semester-attendance-form"
    :validation-schema="schema"
    @submit="getData()"
  >
    <div class="grid grid-cols-3 gap-2">
      <SelectForm name="year" :label="t('tableHead.academicYear')" v-model="academicYear" :options="academicYearOptions" />
      <SelectForm name="month" :label="t('label.semester')" v-model="semester" :options="semesterOptions" />
      <SelectForm name="grade" :label="t('tableHead.grade')" v-model="grade" :options="gradeOptions" />
    </div>
    <div class="flex items-center justify-end">
      <ButtonRoundedWithIcon :label="t('label.confirm')" type="submit" form="semester-attendance-form" color="indigo" :loading="isBusy">
...
import { formatFullDate, getTotalDaysByDateRange, handleErrorResponse } from '@/utils/utilities';
...
const semesterOptions: Ref<ISelectOption[]> = ref([
  {
    label: '1',
    value: '1'
  },
...
const semester: Ref<ISelectOption | null> = ref(null)
...
const getData = async (): Promise<void> => {
  ...
  isBusy.value = true
  rows.value = []
  await attendanceServices.getSemesterAttendance(payload).then((result: AxiosResponse) => {
    const datas = result.data.data.rows
    const dayOff: number = result.data.data.dayOff
    const startDate: string = result.data.data.startDate
    const endDate: string = result.data.data.endDate
    const weekdays: number = getTotalDaysByDateRange(startDate, endDate) - dayOff
    datas.forEach((item: any) => {
      const entry = {
        name: item.fullname,
        nisn: item.nisn,
...
const downloadReports = async () => {
  ...
  await attendanceServices.downloadSemesterAttendance(payload).then((result: AxiosResponse) => {
...
const generateReports = async () => {
  ...
  const response = await fetch('/semester-attendance.html')
  ...
  htmlTemplate.value = htmlTemplate.value.replace('<td id="academic-year"></td>', `<td id="academic-year">${academicYear.value?.label}</td>`)
...
const populateReports = async () => {
  ...
  htmlData.value = ''
  await attendanceServices.getSemesterAttendance(payload).then((result) => {
    const datas = result.data.data.rows
    const dayOff: number = result.data.data.dayOff
    const startDate: string = result.data.data.startDate
    const endDate: string = result.data.data.endDate
