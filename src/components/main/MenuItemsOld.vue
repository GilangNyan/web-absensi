<template>
  <RouterLink :to="href" class="flex px-2 hover:bg-gray-100 text-[13px] text-gray-600" v-if="!hasChild">
  ...
  <div class="flex flex-col overflow-hidden" v-else>
    <a href="#" class="has-child w-full flex px-2 items-center hover:bg-gray-100 text-[13px] text-gray-600 cursor-pointer z-10" @click.prevent="toggleSubmenu()">
      <span class="group flex flex-1 px-4 py-3 space-x-3 items-center">
        <component :is="heroIcons[icon]" class="w-4 h-4" />
        <span class="font-medium">{{ simpleLanguage(title) }}</span>
      </span>
    ...
    <Transition name="slide-down">
      <div class="flex flex-col" v-show="submenus?.length != 0 && submenuOpen">
        <RouterLink :to="item.url" v-for="(item, index) in submenus" :key="index" class="w-full ml-8 pl-5 py-2 text-[13px] text-gray-600 border-l-2 border-gray-300 hover:bg-gray-100">
